...
from tools._llm import _extract_json_block, _get_api_base, _get_model
...
def test_generate_text_delegates_to_messages(
    ...
    monkeypatch.setattr(
        "tools._llm.generate_text_messages", fake_generate_text_messages
    )
...
def test_debug_log_written(
    ...
    fake_resp.json.return_value = {"choices": [{"message": {"content": response_text}}]}
    fake_resp.raise_for_status.return_value = None

    with mock.patch("tools._llm.requests.post", return_value=fake_resp):
