...
async def test_rag_based_analysis():
    ...
    class MockSavepointManager:
        def __init__(self):
            self.savepoint_repo = type(
                "MockRepo", (), {"_current_story_dir": "/tmp/test_story"}
            )()
    ...
    settings = GenerationSettings(
        wanted_chapters=3, debug=True, stream=False, seed=42, log_prompt_inputs=False
    )

    # Test 1: Story context initialization
    print("\n1. Testing story context initialization...")
    try:
        context = await story_state_manager.initialize_story_context(
    ...
    try:
        character_data = await story_state_manager.analyze_character_development_rag(
            "Sarah", settings
        )
