...
class EmbeddingMigrator:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if actual_dimensions != expected_dimensions:
                print(
                    f"⚠️  Warning: Model generates {actual_dimensions} dimensions, config expects {expected_dimensions}"
                )
                print(
                    f"   Updating config to use actual dimensions: {actual_dimensions}"
                )
    ...
    async def analyze_migration(self) -> Dict[str, Any]:
                ...
                if table_exists:
                    ...
                    if chunk_count > 0:
                        ...
                        if result and result > 0:
                            ...
                            print(
                                f"   🔍 Detected current vector dimensions: {current_dimensions}"
                            )
                        else:
                            print(
                                f"   ⚠️  Could not detect vector dimensions, using default: {current_dimensions}"
                            )
                    else:
                        print(
                            f"   📝 No existing embeddings found, using default: {current_dimensions}"
                        )
                else:
                    print(
                        f"   📝 No content_chunks table found, using default: {current_dimensions}"
                    )
        except Exception as e:
            print(
                f"   ⚠️  Error detecting vector dimensions: {e}, using default: {current_dimensions}"
            )
        ...
        print(
            f"   Content types: {', '.join(f'{k}: {v}' for k, v in content_summary.items())}"
        )
    ...
    async def create_migration_table(self) -> str:
        ...
        async with self.new_vector_store._pool.acquire() as conn:
            ...
            try:
                await conn.execute(
                    f"DROP TABLE IF EXISTS {migration_table_name} CASCADE"
                )
            except Exception as e:
            ...
            # Create indexes
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_story_id ON {migration_table_name}(story_id)"
            )
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_content_type ON {migration_table_name}(content_type)"
            )
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_chapter_scene ON {migration_table_name}(chapter_number, scene_number)"
            )
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_created_at ON {migration_table_name}(created_at)"
            )
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_metadata_gin ON {migration_table_name} USING GIN (metadata)"
            )
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_content_type_subtype ON {migration_table_name}(content_type, content_subtype)"
            )

            # Create vector indexes
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_embedding_hnsw ON {migration_table_name} USING hnsw (embedding vector_cosine_ops)"
            )
            await conn.execute(
                f"CREATE INDEX idx_{migration_table_name}_embedding_ivfflat ON {migration_table_name} USING ivfflat (embedding vector_cosine_ops)"
            )
    ...
    async def migrate_content(
        self, migration_table_name: str, dry_run: bool = False
    ) -> Dict[str, Any]:
            ...
            for chunk_id, content_type, content, metadata in chunks:
                try:
                    if not dry_run:
                        ...
                        new_embedding = (
                            await self.embedding_provider.get_single_embedding(content)
                        )
                    ...
                    migration_stats["content_types"][content_type] = (
                        migration_stats["content_types"].get(content_type, 0) + 1
                    )

                    if migration_stats["chunks_migrated"] % 10 == 0:
                        print(
                            f"   ✅ Migrated {migration_stats['chunks_migrated']} chunks..."
                        )
        ...
        print(
            f"   Content types: {', '.join(f'{k}: {v}' for k, v in migration_stats['content_types'].items())}"
        )
    ...
    async def swap_tables(
        ...
        try:
            async with self.new_vector_store._pool.acquire() as conn:
                ...
                old_table_count = await conn.fetchval(
                ...
                if old_table_count == 0:
                    ...
                    await conn.execute(
                        f"ALTER TABLE {migration_table_name} RENAME TO content_chunks"
                    )

                    # Recreate indexes with correct names
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_story_id RENAME TO idx_content_chunks_story_id"
                    )
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_content_type RENAME TO idx_content_chunks_content_type"
                    )
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_chapter_scene RENAME TO idx_content_chunks_chapter_scene"
                    )
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_created_at RENAME TO idx_content_chunks_created_at"
                    )
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_metadata_gin RENAME TO idx_content_chunks_metadata_gin"
                    )
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_content_type_subtype RENAME TO idx_content_chunks_content_type_subtype"
                    )
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_embedding_hnsw RENAME TO idx_content_chunks_embedding_hnsw"
                    )
                    await conn.execute(
                        f"ALTER INDEX idx_{migration_table_name}_embedding_ivfflat RENAME TO idx_content_chunks_embedding_ivfflat"
                    )
                else:
                    print(
                        f"📝 Old table has {old_table_count} chunks, performing full swap..."
                    )
                    # Rename old table to backup
                    await conn.execute(
                        "ALTER TABLE content_chunks RENAME TO content_chunks_backup"
                    )
    ...
    async def record_migration_start(self, migration_info: Dict[str, Any]) -> None:
        ...
        try:
            if self.new_vector_store and self.new_vector_store._pool:
                ...
                print(
                    "   ⚠️  Warning: No vector store pool available for status recording"
                )
        except Exception as e:
    ...
    async def record_migration_completion(
        ...
        try:
            if self.new_vector_store and self.new_vector_store._pool:
                ...
                print(
                    "   ⚠️  Warning: No vector store pool available for completion recording"
                )
        except Exception as e:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate embedding models in RAG system"
    )
    parser.add_argument(
        "--new-model",
        required=True,
    ...
    parser.add_argument(
        "--skip-cleanup", action="store_true", help="Skip cleanup of backup table"
    )
