...
class StoryStateManager:
    ...
    async def initialize_story_context(
    ...
    async def plan_next_chapter(self, settings: GenerationSettings) -> ChapterState:
        ...
        if not self.story_context:
            raise ValueError(
                "Story context must be initialized before planning chapters"
            )
        ...
        chapter_data = self._parse_chapter_plan_response(
            response.content, next_chapter_num
        )
    ...
    async def update_story_evolution(
    ...
    async def revise_chapter_plan(
        ...
        response = await execute_prompt_with_savepoint(
            ...
            variables={
                ...
                "evolution_since_planning": self._get_evolution_since_chapter(
                    chapter_num
                ),
            },
    ...
    async def _analyze_chapter_evolution_rag(
        self, chapter_num: int, settings: GenerationSettings
    ) -> Dict[str, Any]:
        ...
        if character_response:
            evolution_data["character_updates"] = self._extract_list_items(
                character_response
            )
        ...
        if plot_response:
            evolution_data["plot_advancements"] = self._extract_list_items(
                plot_response
            )
        ...
        if world_response:
            evolution_data["world_developments"] = self._extract_list_items(
                world_response
            )
    ...
    async def _query_rag(
        self, query: str, settings: GenerationSettings
    ) -> Optional[str]:
    ...
    def _get_basic_evolution_data(self, chapter_num: int) -> Dict[str, Any]:
        ...
        return {
            "character_updates": [
                f"Chapter {chapter_num} character development analyzed"
            ],
    ...
    def _prepare_planning_context(self) -> str:
        ...
        if self.chapters:
            context_parts.append(
                f"Story Progress: {len(self.chapters)} chapters completed"
            )
            context_parts.append(
                f"Recent Developments: {', '.join(self.story_evolution[-3:])}"
            )
    ...
    def _serialize_characters(self) -> str:
        ...
        for char in self.characters.values():
            char_info.append(
                f"{char.name} ({char.current_role}): {', '.join(char.current_goals)}"
            )
    ...
    def _serialize_plot_threads(self) -> str:
        ...
        for thread in active_threads:
            thread_info.append(
                f"{thread.name}: {thread.description} (Tension: {thread.tension_level}/10)"
            )
    ...
    def _parse_chapter_plan_response(
        self, response: str, chapter_num: int
    ) -> Dict[str, Any]:
    ...
    def _parse_revision_response(self, response: str) -> Dict[str, Any]:
        ...
        for line in lines:
            if line.startswith("Updated Content:"):
                ...
                current_section == "plot" and line.strip() and not line.startswith("-")
            ):
    ...
    def _apply_evolution_updates(
        self, evolution_data: Dict[str, Any], chapter_num: int
    ) -> None:
        ...
        if evolution_data["character_updates"]:
            evolution_summary += (
                f"Character updates: {len(evolution_data['character_updates'])}. "
            )
        if evolution_data["plot_advancements"]:
            evolution_summary += (
                f"Plot advancements: {len(evolution_data['plot_advancements'])}. "
            )
    ...
    async def analyze_character_development_rag(
        ...
        if traits_response:
            character_data["personality_traits"] = self._extract_list_items(
                traits_response
            )
        ...
        if motivations_response:
            character_data["motivations"] = self._extract_list_items(
                motivations_response
            )
        ...
        if relationships_response:
            ...
            character_data["relationships"] = self._parse_relationships(
        ...
        growth_query = (
            f"What key moments of growth or change has {character_name} experienced?"
        )
    ...
    async def analyze_plot_threads_rag(
        ...
        if involvement_response:
            plot_data["character_involvement"] = self._parse_character_involvement(
    ...
    def _get_basic_character_data(self, character_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "personality_traits": [
                f"{character_name} has developed personality traits"
            ],
    ...
    def _apply_chapter_revisions(
        self, chapter_state: ChapterState, revision_data: Dict[str, Any]
    ) -> None:
        ...
        if revision_data["character_updates"]:
            chapter_state.character_developments.extend(
                revision_data["character_updates"]
            )
    ...
    def _save_state(self) -> None:
        ...
        state_data = {
            "story_context": {
                "story_direction": self.story_context.story_direction
                if self.story_context
                else "",
                "current_themes": self.story_context.current_themes
                if self.story_context
                else [],
                "world_rules": self.story_context.world_rules
                if self.story_context
                else [],
                "tone_style": self.story_context.tone_style
                if self.story_context
                else "",
                "target_audience": self.story_context.target_audience
                if self.story_context
                else "",
                "genre_conventions": self.story_context.genre_conventions
                if self.story_context
                else [],
                "story_pacing": self.story_context.story_pacing
                if self.story_context
                else "medium",
                "current_tension": self.story_context.current_tension
                if self.story_context
                else 1,
                "story_goals": self.story_context.story_goals
                if self.story_context
                else [],
                "completed_arcs": self.story_context.completed_arcs
                if self.story_context
                else [],
            },
            "characters": {
        ...
        try:
            ...
            if "characters" in state_data:
                for name, char_data in state_data["characters"].items():
                    char_data["created_at"] = datetime.fromisoformat(
                        char_data["created_at"]
                    )
                    char_data["updated_at"] = datetime.fromisoformat(
            ...
            if "chapters" in state_data:
                for num, chapter_data in state_data["chapters"].items():
                    chapter_data["created_at"] = datetime.fromisoformat(
                        chapter_data["created_at"]
                    )
                    chapter_data["updated_at"] = datetime.fromisoformat(
