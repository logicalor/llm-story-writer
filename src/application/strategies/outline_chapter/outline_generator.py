...
class OutlineGenerator:
    ...
    async def generate_outline(
        ...
        try:
            ...
            prompt_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "multistep/outline/understand_prompt",
                {
                    "prompt": prompt,
                },
            )
            ...
            story_analysis = await self._generate_story_analysis_chunks(
                prompt, settings, conversation_history
            )

            # Extract story start date from core foundation chunk
            start_date = await self._extract_story_start_date_from_chunks(
                story_analysis, settings
            )

            # Extract base context from core foundation chunk
            base_context = await self._extract_base_context_from_chunks(
                story_analysis, settings
            )
            ...
            await self.setting_manager.generate_setting_sheets(
                story_elements, base_context, settings
            )
    ...
    async def initialize_progressive_outline(
        ...
        try:
            if settings.debug:
                print(
                    "[PROGRESSIVE PLANNING] Initializing progressive outline system..."
                )

            # Initialize story context using story state manager
            story_context = await self.story_state_manager.initialize_story_context(
                prompt, settings
            )

            if settings.debug:
                print(
                    f"[PROGRESSIVE PLANNING] Story context initialized: {story_context.story_direction}"
                )

            # Generate initial story analysis chunks for RAG indexing
            story_chunks = await self._generate_story_analysis_chunks(
                prompt, settings, []
            )
            ...
            raise StoryGenerationError(
                f"Failed to initialize progressive outline: {e}"
            ) from e

    async def _index_story_analysis_chunk(
    ...
    async def _generate_story_analysis_chunks(
        self, prompt: str, settings: GenerationSettings, conversation_history: list
    ) -> Dict[str, str]:
        ...
        try:
            ...
            ] = await self._generate_core_story_foundation_chunk(
                prompt, settings, base_conversation
            )
            chunks[
                "character_foundation"
            ] = await self._generate_character_foundation_chunk(
                prompt, settings, base_conversation
            )
            chunks[
                "setting_foundation"
            ] = await self._generate_setting_foundation_chunk(
                prompt, settings, base_conversation
            )
            chunks["plot_structure"] = await self._generate_plot_structure_chunk(
                prompt, settings, base_conversation
            )
            chunks["theme_message"] = await self._generate_theme_message_chunk(
                prompt, settings, base_conversation
            )
            chunks["tone_style"] = await self._generate_tone_style_chunk(
                prompt, settings, base_conversation
            )
            chunks["conflict_stakes"] = await self._generate_conflict_stakes_chunk(
                prompt, settings, base_conversation
            )
            chunks["world_rules_logic"] = await self._generate_world_rules_logic_chunk(
                prompt, settings, base_conversation
            )
            ...
            if settings.debug:
                print(
                    f"[STORY ANALYSIS] Generated all {len(chunks)} story analysis chunks"
                )
    ...
    async def _generate_core_story_foundation_chunk(
        ...
        try:
            ...
            await self._index_story_analysis_chunk(
                content, "core_story_foundation", settings
            )
            ...
            if settings.debug:
                print(
                    f"[STORY ANALYSIS] Error generating core story foundation chunk: {e}"
                )
            return ""
    ...
    async def _generate_character_foundation_chunk(
        ...
        try:
            ...
            # Index this chunk immediately
            await self._index_story_analysis_chunk(
                content, "character_foundation", settings
            )
            ...
            if settings.debug:
                print(
                    f"[STORY ANALYSIS] Error generating character foundation chunk: {e}"
                )
            return ""
    ...
    async def _generate_setting_foundation_chunk(
        ...
        try:
            ...
            # Index this chunk immediately
            await self._index_story_analysis_chunk(
                content, "setting_foundation", settings
            )
            ...
            if settings.debug:
                print(
                    f"[STORY ANALYSIS] Error generating setting foundation chunk: {e}"
                )
            return ""
    ...
    async def _generate_world_rules_logic_chunk(
        ...
        try:
            ...
            # Index this chunk immediately
            await self._index_story_analysis_chunk(
                content, "world_rules_logic", settings
            )
    ...
    async def _extract_story_start_date_from_chunks(
        self, story_analysis: Dict[str, str], settings: GenerationSettings
    ) -> str:
        ...
        try:
            ...
            model_config = ModelConfig.from_string(
                self.config["models"]["creative_model"]
            )
            ...
            if settings.debug:
                print(
                    f"[STORY START DATE] Error extracting start date from chunks: {e}"
                )
            return "Present day"  # Default fallback
    ...
    async def _generate_story_elements_from_chunks(
        ...
        try:
            ...
            if settings.debug:
                print(
                    f"[STORY ELEMENTS] Error generating story elements from chunks: {e}"
                )
            return "Story elements generation in progress"  # Default fallback
