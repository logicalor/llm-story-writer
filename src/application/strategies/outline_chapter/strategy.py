    ...
    async def _setup_savepoints(self, prompt_filename: str) -> None:
        ...
        if self.savepoint_repo:
            ...
            self.savepoint_manager = SavepointManager(
                self.savepoint_repo, prompt_filename
            )
            ...
            self.outline_generator.character_manager.savepoint_manager = (
                self.savepoint_manager
            )
            self.outline_generator.setting_manager.savepoint_manager = (
                self.savepoint_manager
            )
            self.chapter_generator.character_manager.savepoint_manager = (
                self.savepoint_manager
            )
            self.chapter_generator.setting_manager.savepoint_manager = (
                self.savepoint_manager
            )
            self.chapter_generator.recap_manager.savepoint_manager = (
                self.savepoint_manager
            )
            self.chapter_generator.scene_generator.savepoint_manager = (
                self.savepoint_manager
            )
...
You have deep knowledge of storytelling techniques, character development, plot structure, and creative writing principles. Use this expertise to create high-quality story content."""
    ...
    async def generate_progressive_story(
        ...
        try:
            ...
            init_result = await self.outline_generator.initialize_progressive_outline(
                prompt, settings
            )
            print(
                f"[PROGRESSIVE STORY] Progressive outline initialized: {init_result['status']}"
            )
            ...
            while chapter_count < settings.wanted_chapters:
                ...
                chapter_plan = (
                    await self.chapter_generator.plan_next_chapter_progressive(settings)
                )
                print(
                    f"[PROGRESSIVE STORY] Chapter {chapter_count} planned: {chapter_plan['title']}"
                )

                # Generate the chapter content using ChapterGenerator
                print(
                    f"[PROGRESSIVE STORY] Generating Chapter {chapter_count} content..."
                )
                ...
                print(
                    f"[PROGRESSIVE STORY] Analyzing Chapter {chapter_count} evolution..."
                )
                await self.story_state_manager.update_story_evolution(
                    chapter_count, settings
                )

                # Check if we need to revise previous chapter plans
                if chapter_count > 1:
                    print(
                        f"[PROGRESSIVE STORY] Checking if Chapter {chapter_count - 1} needs revision..."
                    )
                    await self._check_and_revise_previous_chapters(
                        chapter_count, settings
                    )

                print(
                    f"[PROGRESSIVE STORY] Chapter {chapter_count} completed. Story state updated."
                )
                print(
                    f"[PROGRESSIVE STORY] Current story summary:\n{self.story_state_manager.get_story_summary()}"
                )
            ...
            raise StoryGenerationError(
                f"Failed to generate progressive story: {e}"
            ) from e

    async def get_progressive_outline_status(
        ...
        try:
            ...
            planned_chapters = len(
                [
                    c
                    for c in self.story_state_manager.chapters.values()
                    if c.status == "planned"
                ]
            )
            ...
            return {
                ...
                "plot_threads": len(
                    [
                        t
                        for t in self.story_state_manager.plot_threads.values()
                        if t.status == "active"
                    ]
                ),
            ...
            raise StoryGenerationError(
                f"Failed to get progressive outline status: {e}"
            ) from e
    ...
    async def plan_next_chapter_progressive(
        ...
        try:
            if not self.story_state_manager.story_context:
                raise StoryGenerationError(
                    "Progressive outline system not initialized. Call generate_progressive_story first."
                )

            # Use ChapterGenerator to coordinate chapter planning
            chapter_plan = await self.chapter_generator.plan_next_chapter_progressive(
                settings
            )

            if settings.debug:
                print(
                    f"[PROGRESSIVE PLANNING] Next chapter planned: {chapter_plan['title']}"
                )
    ...
    async def _generate_chapter_from_plan(
        self, chapter_plan: Dict[str, Any], settings: GenerationSettings
    ) -> Chapter:
        ...
        try:
            ...
            raise StoryGenerationError(
                f"Failed to generate chapter from plan: {e}"
            ) from e

    async def _check_and_revise_previous_chapters(
        self, current_chapter: int, settings: GenerationSettings
    ) -> None:
        ...
        try:
            ...
            if prev_chapter in self.story_state_manager.chapters:
                ...
                if current_chapter - chapter_state.chapter_number > 2:
                    print(
                        f"[PROGRESSIVE STORY] Revising Chapter {prev_chapter} plan..."
                    )
                    # Use ChapterGenerator to coordinate revision
                    await self.chapter_generator.revise_outline_progressive(
                        prev_chapter, settings
                    )
                    print(f"[PROGRESSIVE STORY] Chapter {prev_chapter} plan revised.")

        except Exception as e:
            print(
                f"[PROGRESSIVE STORY] Warning: Could not check/revise previous chapters: {e}"
            )

    async def generate_story_info(
        self, outline: Outline, chapters: List[Chapter], settings: GenerationSettings
    ) -> StoryInfo:
