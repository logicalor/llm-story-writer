...
class SceneGenerator:
        ...
        try:
            if not self.savepoint_manager:
                raise StoryGenerationError(
                    "Savepoint manager is required for scene generation"
                )

            # Parse the chapter outline to extract scene definitions
            scene_definitions = await self._parse_scene_definitions(
                chapter_num, chapter_outline, settings
            )
            ...
            if not scene_definitions:
                ...
                scene_definitions = [
                    {
                        "title": f"Chapter {chapter_num} Scene",
            ...
            for scene_num, scene_def in enumerate(scene_definitions, 1):
                try:
                    print(
                        f"    Generating Scene {scene_num} for Chapter {chapter_num}..."
                    )
                    ...
                    if await self.savepoint_manager.has_step(scene_savepoint_id):
                        ...
                        scene_content = await self.savepoint_manager.load_step(
                            scene_savepoint_id
                        )
                    ...
                    await self.savepoint_manager.save_step(
                        scene_savepoint_id, scene_content
                    )
                    await self.savepoint_manager.save_step(
                        f"chapter_{chapter_num}/scene_{scene_num}_title", scene_title
                    )
    ...
    async def _parse_scene_definitions(
        self, chapter_num: int, chapter_outline: str, settings: GenerationSettings
    ) -> List[dict]:
            ...
            try:
                ...
                for scene in scene_definitions:
                    if (
                        ...
                        or "title" not in scene
                        or "description" not in scene
                    ):
                        raise ValueError(
                            "Each scene must be an object with title and description"
                        )

                if settings.debug:
                    print(
                        f"[SCENE DEFINITIONS] Successfully parsed {len(scene_definitions)} scenes from JSON"
                    )

            except (json.JSONDecodeError, ValueError) as e:
                if settings.debug:
                    print(
                        f"[SCENE DEFINITIONS] JSON parsing failed: {e}, falling back to default structure"
                    )
                scene_definitions = []
        else:
            if settings.debug:
                print(
                    f"[SCENE DEFINITIONS] JSON parsing failed: {response.json_errors}, falling back to default structure"
                )
            scene_definitions = []
    ...
    async def _get_scene_outline_summary(
        ...
        try:
            ...
            response = await execute_prompt_with_savepoint(
                ...
                variables={"scene_definition": scene_definition, "tense": tense},
            ...
            if settings.debug:
                print(
                    f"[SCENE OUTLINE SUMMARY] Generated {tense} tense summary: {len(summary)} characters"
                )
        ...
        try:
            ...
            if key_events:
                if isinstance(key_events, list):
                    event_bullets = "\n".join(
                        [f"• {str(event)}" for event in key_events]
                    )
            ...
            if settings.debug:
                print(
                    f"[SCENE OUTLINE SUMMARY PROGRAMMATIC] Generated summary: {len(summary)} characters"
                )
            ...
            if settings.debug:
                print(
                    f"[SCENE OUTLINE SUMMARY PROGRAMMATIC] Error generating summary: {e}"
                )
            return f"Error generating scene summary: {e}"
    ...
    async def _get_chapter_outline_summary(
        ...
        try:
            ...
            response = await execute_prompt_with_savepoint(
                ...
                variables={"chapter_outline": chapter_outline, "tense": tense},
            ...
            if settings.debug:
                print(
                    f"[CHAPTER OUTLINE SUMMARY] Generated {tense} tense summary: {len(summary)} characters"
                )
    ...
    async def _generate_scene_content_multistep(
        ...
        instructions_config = ModelConfig.from_string(
            self.config["models"]["scene_writer"]
        )
        scene_writer_config = ModelConfig.from_string(
            self.config["models"]["scene_writer"]
        )
        ...
        try:
            ...
            if isinstance(scene_data, dict) and "characters" in scene_data:
                ...
                if settings.debug:
                    print(
                        "[MULTISTEP SCENE] No 'characters' property found in scene definition or invalid format"
                    )
        except (json.JSONDecodeError, AttributeError) as e:
            if settings.debug:
                print(f"[MULTISTEP SCENE] Error parsing scene definition JSON: {e}")
                print(
                    f"[MULTISTEP SCENE] Scene definition: {scene_definition[:200]}..."
                )
            character_names = []
        ...
        if character_names:
            try:
                character_summaries = (
                    await self.character_manager.get_character_summaries(
                        character_names, settings
                    )
                )
                ...
                if settings.debug:
                    print(
                        f"[MULTISTEP SCENE] Retrieved summaries for {len(character_names)} characters: {character_names}"
                    )
            except Exception as e:
                if settings.debug:
        ...
        try:
            ...
            if isinstance(scene_data, dict) and "setting" in scene_data:
                ...
                if settings.debug:
                    print(
                        "[MULTISTEP SCENE] No 'setting' property found in scene definition or invalid format"
                    )
        except (json.JSONDecodeError, AttributeError) as e:
            if settings.debug:
                print(
                    f"[MULTISTEP SCENE] Error parsing scene definition for settings: {e}"
                )
            setting_names = []
        ...
        if setting_names:
            try:
                setting_summary = await self.setting_manager.get_setting_summaries(
                ...
                if settings.debug:
                    print(
                        f"[MULTISTEP SCENE] Retrieved summaries for {len(setting_names)} settings: {setting_names}"
                    )
            except Exception as e:
                if settings.debug:
        ...
        try:
            ...
            if settings.debug:
                ...
                print(
                    f"[MULTISTEP SCENE] System message loaded: {len(system_message)} characters"
                )
            ...
            try:
                ...
                if settings.debug:
                    print(
                        "[MULTISTEP SCENE] Step 1: First message added (understand elements)"
                    )
                    print(
                        f"[MULTISTEP SCENE] Message length: {len(elements_prompt)} characters"
                    )
            ...
            try:
                ...
                if settings.debug:
                    print(
                        "[MULTISTEP SCENE] Step 2: Second message added (understand context)"
                    )
                    print(
                        f"[MULTISTEP SCENE] Message length: {len(context_prompt)} characters"
                    )
            ...
            if (
                character_names
            ):  # Only continue if previous step succeeded and characters exist
                try:
                    ...
                    if settings.debug:
                        print(
                            "[MULTISTEP SCENE] Step 3: Third message added (understand characters)"
                        )
                        print(
                            f"[MULTISTEP SCENE] Message length: {len(characters_prompt)} characters"
                        )
                ...
                print(
                    "[MULTISTEP SCENE] Step 3 skipped - no characters found in scene definition"
                )

            # Step 4: Fourth user message (understand setting) - only if settings exist
            if (
                setting_names
            ):  # Only continue if previous step succeeded and settings exist
                try:
                    ...
                    if settings.debug:
                        print(
                            "[MULTISTEP SCENE] Step 4: Fourth message added (understand setting)"
                        )
                        print(
                            f"[MULTISTEP SCENE] Message length: {len(setting_prompt)} characters"
                        )
                ...
                print(
                    "[MULTISTEP SCENE] Step 4 skipped - no settings found in scene definition"
                )
            ...
            if scene_num == 1 or scene_num != chapter_count:
                try:
                    ...
                    conversation_history.append(
                        {"role": "user", "content": understand_next_scene_prompt}
                    )
            ...
            if scene_num == chapter_count or scene_num != 1:
                try:
                    ...
                    conversation_history.append(
                        {"role": "user", "content": understand_previous_scene_prompt}
                    )
            ...
            if scene_num == chapter_count:
                try:
                    ...
                    conversation_history.append(
                        {"role": "user", "content": understand_next_chapter_prompt}
                    )
            ...
            scene_outline = await self._get_scene_outline_summary_programmatic(
                scene_definition=scene_definition, settings=settings
            )
