...
class ModelRerankerService:
    ...
    def __init__(
        self, model_name: str = "BAAI/bge-reranker-v2-m3", use_gpu: bool = False
    ):
    ...
    async def rerank_results(
        ...
        if strategy == "cross_encoder":
            ...
            logger.warning(
                f"Unknown reranking strategy: {strategy}, using cross_encoder"
            )
            return await self._cross_encoder_rerank(query, parsed_results, max_length)

    async def _cross_encoder_rerank(
        self, query: str, results: List[Dict[str, Any]], max_length: int
    ) -> List[ModelRerankedResult]:
        ...
        try:
            ...
            scores = await loop.run_in_executor(
                self._executor, self._run_cross_encoder_inference, pairs
            )
    ...
    async def _hybrid_rerank(
        ...
        try:
            ...
            cross_encoder_results = await self._cross_encoder_rerank(
                query, results, max_length
            )

            # Combine scores: 70% cross-encoder, 30% original similarity
            for result in cross_encoder_results:
                combined_score = (result.reranked_score * 0.7) + (
                    result.original_similarity * 0.3
                )
