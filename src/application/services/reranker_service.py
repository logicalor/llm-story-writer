...
class RerankerService:
    ...
    async def rerank_results(
        self, query: str, results: List[Tuple], strategy: str = "hybrid"
    ) -> List[RerankedResult]:
        """
    ...
    async def _hybrid_rerank(
        ...
        for result in results:
            ...
            content_type_score = self._calculate_content_type_score(
                query, result["content_type"]
            )
            ...
            reranked.append(
                RerankedResult(
                    ...
                    reranking_reason=self._generate_reranking_reason(
                        result["similarity"],
                        keyword_score if self.use_keyword_boost else 0,
                        metadata_score if self.use_metadata_boost else 0,
                        content_type_score,
                    ),
                )
            )
    ...
    def _calculate_content_type_score(self, query: str, content_type: str) -> float:
        ...
        if any(
            word in query_lower
            for word in ["character", "person", "protagonist", "hero", "villain"]
        ):
        ...
        if any(
            word in query_lower
            for word in ["setting", "location", "place", "world", "environment"]
        ):
        ...
        if any(
            word in query_lower
            for word in ["plot", "story", "narrative", "conflict", "theme"]
        ):
