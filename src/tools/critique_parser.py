    ...
    def parse_critique(self, critic_type: str, response: str) -> CritiqueResult:
        ...
        try:
            ...
            for criterion, max_score in criteria.items():
                score_data = self._extract_criterion_score(
                    response, criterion, max_score
                )
                if score_data:
            ...
            raise StoryGenerationError(
                f"Failed to parse critique from {critic_type}: {e}"
            ) from e

    def _extract_criterion_score(
        self, response: str, criterion: str, max_score: int
    ) -> Optional[CritiqueScore]:
    ...
    def format_critique_feedback(self, critique_results: List[CritiqueResult]) -> str:
        ...
        for result in critique_results:
            feedback_parts.append(
                f"## {result.critic_type.replace('-', ' ').title()} Critique"
            )
            ...
            for score in result.scores:
                feedback_parts.append(
                    f"### {score.criterion}: {score.score}/{score.max_score} ({score.percentage:.1f}%)"
                )
