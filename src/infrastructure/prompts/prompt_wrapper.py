    ...
    try:
        ...
        try:
            ...
            if end_index > start_index:
                ...
                print(
                    "⚠️ Warning: Found unclosed <output> tag, extracting content to end of text"
                )
                return content
        except ValueError:
...
def validate_and_parse_output(
    text: str, skip_validation: bool = False
) -> tuple[str, bool]:
    """
...
async def execute_prompt_with_savepoint(
    ...
    **kwargs,
    ...
    if response.was_cached:
        ...
        print(
            f"📋 Using cached content from savepoint '{savepoint_id}' - skipping validation/parsing"
        )
    else:
        # Apply output validation and parsing for newly generated content
        parsed_content, needs_retry = validate_and_parse_output(
            response.content, skip_validation
        )
        if needs_retry:
            ...
            parsed_content, needs_retry = validate_and_parse_output(
                retry_response.content, skip_validation
            )
            if needs_retry:
                print(
                    f"⚠️ Warning: Retry failed for prompt '{prompt_id}', using raw output"
                )
    ...
    print(
        f"[PROMPT TIMING] {prompt_id} | Model: {model_name} | Duration: {duration:.2f}s{savepoint_info}"
    )
...
async def execute_prompt(
    ...
    **kwargs,
    ...
    model_name = model_config.name if model_config else "unknown"
    print(
        f"[PROMPT TIMING] {prompt_id} | Model: {model_name} | Duration: {duration:.2f}s"
    )
...
async def execute_messages_with_savepoint(
    ...
    **kwargs,
    ...
    if savepoint_id and handler.savepoint_repo and not force_regenerate:
        if await handler.savepoint_repo.has_savepoint(savepoint_id):
            ...
            if cached_content is not None:
                ...
                model_name = model_config.name if model_config else "unknown"
                print(
                    f"[MESSAGES TIMING] {len(conversation_history)} messages | Model: {model_name} | Duration: {duration:.2f}s (cached)"
                )
    ...
    try:
        ...
        parsed_content, needs_retry = validate_and_parse_output(
            response_content, skip_validation
        )
        if needs_retry:
        ...
        print(
            f"[MESSAGES TIMING] {message_count} messages | Model: {model_name} | Duration: {duration:.2f}s{savepoint_info}"
        )
        ...
        model_name = model_config.name if model_config else "unknown"
        print(
            f"[MESSAGES TIMING] {len(conversation_history)} messages | Model: {model_name} | Duration: {duration:.2f}s (ERROR)"
        )
...
def load_prompt(
    handler: PromptHandler, prompt_id: str, variables: Optional[Dict[str, Any]] = None
) -> str:
    """
    ...
    try:
        ...
        if variables:
            try:
                ...
                print(
                    f"[PROMPT LOADER] Warning: Error applying variables to prompt {prompt_id}: {e}"
                )
