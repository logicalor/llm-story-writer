...
class LlamaCppProvider(ModelProvider):
    ...
    async def _generate_text_non_streaming(
        ...
        try:
            ...
            if word_count < min_word_count:
                ...
                if response.status_code != 200:
                    raise ModelProviderError(
                        f"llama.cpp API error on continuation: {response.status_code} - {response.text}"
                    )
    ...
    async def stream_text(
        ...
        try:
            ...
            response = await asyncio.to_thread(
                requests.post, url, json=payload, timeout=300, stream=True
            )
    ...
    async def is_model_available(self, model_config: ModelConfig) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return any(
                    model.get("name", "").startswith(model_config.name)
    ...
    async def download_model(self, model_config: ModelConfig) -> None:
        ...
        print(
            f"Please ensure the model {model_config.name} is available at {model_config.host or self.host}"
        )
    ...
    def _prepare_payload(
        ...
        # If n_ctx is specified but exceeds infrastructure limit, cap it
            ...
            print(
                f"[LLAMA.CPP] Warning: Requested n_ctx {payload['n_ctx']} exceeds infrastructure limit {self.context_length}, capping to {self.context_length}"
            )
            payload["n_ctx"] = self.context_length
        ...
        if seed is not None:
            ...
            if should_randomize:
                ...
                print(
                    f"[LLAMA.CPP] Seed randomized: {seed} + {random_offset} = {payload['seed']}"
                )
            else:
        ...
        if "n_ctx" in payload:
            ...
            print(
                f"[LLAMA.CPP] Context length: {payload['n_ctx']} (from infrastructure default)"
            )
    ...
    async def generate_multistep_conversation(
        ...
        try:
            ...
            raise ModelProviderError(
                f"llama.cpp multi-step conversation failed: {e}"
            ) from e
    ...
    async def _generate_multistep_conversation_with_streaming(
        ...
        try:
            ...
            raise ModelProviderError(
                f"llama.cpp multi-step conversation streaming failed: {e}"
            ) from e
