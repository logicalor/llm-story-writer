...
class CLIApplication:
    ...
    async def _generate_story(self, prompt: str, args):
        ...
        self.logger.log_generation_start(
            prompt_hash=str(hash(prompt)), settings=generation_settings.to_dict()
        )

        # Generate story
        self.logger.info("Starting story generation")
        story = await story_service.generate_story(
            prompt=prompt, settings=generation_settings, prompt_filename=prompt_filename
        )

        # Save story
        self.logger.info("Saving story")
        output_path = await story_service.save_story(story=story, output_path=None)
