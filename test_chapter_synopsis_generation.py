...
from src.infrastructure.storage.savepoint_repository import (
    FilesystemSavepointRepository as SavepointRepository,
)
...
async def test_chapter_synopsis_generation():
    ...
    # Test settings
    settings = GenerationSettings(seed=42, debug=True, stream=False)
    ...
    try:
        ...
        outline = await strategy.generate_outline(
            test_prompt, settings, "test_synopsis"
        )
        ...
        for i in range(1, 5):  # Check for chapters 1-4
            try:
                synopsis = await savepoint_repo.load_step(
                    f"test_synopsis/chapter_{i}/base_synopsis"
                )
