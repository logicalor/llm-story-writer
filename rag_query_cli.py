...
Examples:
  ...
  python rag_query_cli.py --story 1 --interactive --rerank
    ...
    parser.add_argument(
        "--list-stories", "-l", action="store_true", help="List all available stories"
    )
    parser.add_argument("--story", "-s", type=int, help="Story ID to work with")
    parser.add_argument(
        "--summary", action="store_true", help="Show story content summary"
    )
    parser.add_argument("--stats", action="store_true", help="Show content statistics")
    parser.add_argument("--search", type=str, help="Search query to execute")
    parser.add_argument("--query", type=str, help="AI query question")
    parser.add_argument(
    ...
    parser.add_argument(
        "--rerank-type",
        choices=["rule_based", "model_based"],
        default="rule_based",
        help="Type of reranker to use (default: rule_based)",
    )
    parser.add_argument(
        "--rerank-strategy",
        choices=["hybrid", "keyword", "metadata", "semantic", "cross_encoder"],
        default="hybrid",
        help="Reranking strategy to use (default: hybrid)",
    )
    ...
    try:
        ...
        content_chunker = ContentChunker(
            max_chunk_size=config.max_chunk_size, overlap_size=config.overlap_size
        )
...
async def search_story_content(
    ...
    try:
        ...
        for i, (chunk_id, content_type, content, metadata, similarity) in enumerate(
            results, 1
        ):
    ...
    try:
        ...
        for chunk_id, content_type, content, metadata, similarity in results:
            context_parts.append(
                f"[{content_type.upper()}] (Similarity: {similarity:.3f})\n{content}"
            )
...
async def interactive_conversation(
    ...
    try:
        summary = await rag_service.get_story_summary(story_id)
        story_name = summary.get("story_info", {}).get(
    ...
    while True:
        try:
            ...
            search_threshold = min(
                threshold, 0.3
            )  # Use lower threshold for interactive
