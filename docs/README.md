...
- **Headless orchestrator slice**: `src/presentation/orchestrator.py` now runs the implemented Python-native phase sequence (`init → story-foundation → outline → [outline-critique] → [outline gate] → narrative-arc → characters → settings → wiki-init → wiki-bootstrap → chapter-loop → final-edit → assembly`), persists `PipelineState` savepoints for resume support, stores advisory arc results in `state.arc_result`, stores outline-critique outputs in `state.critic_summary`, `state.arc_distribution`, and `state.promise_payoff` when enabled, records granular work-item checkpoints in `state.completed_work_items` for the characters phase, settings phase, and per-scene chapter drafting path, generates character and setting sheet JSON files under `stories/<story>/characters/` and `stories/<story>/settings/` with populated `summary`, `abridged`, and `chunks` fields plus cached `_names.json` extraction results, writes scene decomposition and per-scene markdown under `stories/<story>/chapters/` when the scene pipeline is active, records per-chapter sheet evolution in `state.evolved_sheets`, writes approved chapter files to `stories/<story>/chapters/chapter_{N}.md`, emits `stories/<story>/output/story_edited.md` when Phase 9 runs, and assembles `stories/<story>/output/story.md` from the final `state.approved_chapters`
...
- [Granular Checkpointing](./features/granular-checkpointing.md) — ADR 010 work-item ledger behavior for characters, settings, and per-scene chapter drafting, including resume-after-kill semantics and work-item ID coverage
