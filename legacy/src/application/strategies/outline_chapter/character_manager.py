...
class CharacterManager:
    ...
    async def generate_character_sheets(
        self, story_elements: str, additional_context: str, settings: GenerationSettings
    ) -> None:
        """Generate character sheets for all characters identified in story elements."""
        try:
            # Extract character names from story elements
            character_names = await self.extract_character_names(
                story_elements, settings
            )

            if settings.debug:
                print(
                    f"[CHARACTER SHEETS] Found {len(character_names)} characters: {character_names}"
                )
            ...
            for character_name in character_names:
                ...
                await self.generate_single_character_sheet(
                    character_name, story_elements, additional_context, settings
                )
        ...
        if response.json_parsed:
            ...
            try:
                ...
                if isinstance(parsed_names, list):
                    ...
                    if settings.debug:
                        print(
                            f"[CHARACTER NAMES] Successfully parsed JSON: {character_names}"
                        )
                else:
                    if settings.debug:
                        print(
                            f"[CHARACTER NAMES] Expected list but got: {type(parsed_names)}"
                        )
            except (json.JSONDecodeError, AttributeError) as e:
                if settings.debug:
                    print(
                        f"[CHARACTER NAMES] JSON parsing failed: {e}, falling back to line parsing"
                    )
                character_names = []
        else:
            if settings.debug:
                print(
                    f"[CHARACTER NAMES] JSON parsing failed: {response.json_errors}, falling back to line parsing"
                )
    ...
    async def generate_single_character_sheet(
        self,
        character_name: str,
        story_elements: str,
        ...
        try:
            ...
            character_creation_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create",
                {
            ...
            conversation = [
                ...
                {"role": "user", "content": character_creation_prompt},
            ]
            ...
            await self._generate_character_chunks(
                character_name, conversation, settings
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER SHEET] Error generating sheet for {character_name}: {e}"
                )
    ...
    async def _generate_character_chunks(
        ...
        try:
            ...
            await self._generate_personality_chunk(
                character_name, conversation.copy(), settings
            )
            await self._generate_background_chunk(
                character_name, conversation.copy(), settings
            )
            await self._generate_motivations_chunk(
                character_name, conversation.copy(), settings
            )
            await self._generate_relationships_chunk(
                character_name, conversation.copy(), settings
            )
            await self._generate_skills_chunk(
                character_name, conversation.copy(), settings
            )
            await self._generate_current_state_chunk(
                character_name, conversation.copy(), settings
            )
            await self._generate_growth_arc_chunk(
                character_name, conversation.copy(), settings
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNKS] Error generating chunks for {character_name}: {e}"
                )
    ...
    async def _generate_personality_chunk(
        ...
        try:
            ...
            personality_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create_personality_chunk",
                {
                    "character_name": character_name,
                },
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Generated personality chunk for {character_name}"
                )

            # Index this chunk immediately after generation
            await self._index_character_chunk(
                character_name, "personality_chunk", settings
            )

        except Exception as e:
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Error generating personality chunk for {character_name}: {e}"
                )

    async def _index_character_chunk(
        self, character_name: str, chunk_type: str, settings: GenerationSettings
    ) -> None:
        ...
        try:
            ...
            if chunk_content:
                ...
                if settings.debug:
                    print(
                        f"[RAG CHARACTER INDEXING] Indexed {len(chunk_ids)} chunks for {character_name} - {chunk_type}"
                    )
        except Exception as e:
            if settings.debug:
                print(
                    f"[RAG CHARACTER INDEXING] Could not index {chunk_type} for {character_name}: {e}"
                )
    ...
    async def _generate_background_chunk(
        ...
        try:
            ...
            background_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create_background_chunk",
                {
                    "character_name": character_name,
                },
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Generated background chunk for {character_name}"
                )

            # Index this chunk immediately after generation
            await self._index_character_chunk(
                character_name, "background_chunk", settings
            )

        except Exception as e:
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Error generating background chunk for {character_name}: {e}"
                )
    ...
    async def _generate_motivations_chunk(
        ...
        try:
            ...
            motivations_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create_motivations_chunk",
                {
                    "character_name": character_name,
                },
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Generated motivations chunk for {character_name}"
                )

            # Index this chunk immediately after generation
            await self._index_character_chunk(
                character_name, "motivations_chunk", settings
            )

        except Exception as e:
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Error generating motivations chunk for {character_name}: {e}"
                )
    ...
    async def _generate_relationships_chunk(
        ...
        try:
            ...
            relationships_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create_relationships_chunk",
                {
                    "character_name": character_name,
                },
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Generated relationships chunk for {character_name}"
                )

            # Index this chunk immediately after generation
            await self._index_character_chunk(
                character_name, "relationships_chunk", settings
            )

        except Exception as e:
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Error generating relationships chunk for {character_name}: {e}"
                )
    ...
    async def _generate_skills_chunk(
        ...
        try:
            ...
            skills_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create_skills_chunk",
                {
                    "character_name": character_name,
                },
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Error generating skills chunk for {character_name}: {e}"
                )
    ...
    async def _generate_current_state_chunk(
        ...
        try:
            ...
            current_state_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create_current_state_chunk",
                {
                    "character_name": character_name,
                },
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Generated current state chunk for {character_name}"
                )

            # Index this chunk immediately after generation
            await self._index_character_chunk(
                character_name, "current_state_chunk", settings
            )

        except Exception as e:
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Error generating current state chunk for {character_name}: {e}"
                )
    ...
    async def _generate_growth_arc_chunk(
        ...
        try:
            ...
            growth_arc_prompt = self.prompt_handler.prompt_loader.load_prompt(
                "characters/create_growth_arc_chunk",
                {
                    "character_name": character_name,
                },
            )
            ...
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Generated growth arc chunk for {character_name}"
                )

            # Index this chunk immediately after generation
            await self._index_character_chunk(
                character_name, "growth_arc_chunk", settings
            )

        except Exception as e:
            if settings.debug:
                print(
                    f"[CHARACTER CHUNK] Error generating growth arc chunk for {character_name}: {e}"
                )

    async def index_character_in_rag(
        self, character_name: str, settings: GenerationSettings
    ) -> None:
        ...
        try:
            ...
            try:
                deleted_count = (
                    await self.rag_integration.cleanup_content_by_type_and_metadata(
                        content_type="character",
                        metadata_filters={"character_name": character_name},
                    )
                )
                if settings.debug:
                    print(
                        f"[RAG CHARACTER INDEXING] Cleaned up {deleted_count} existing chunks for character '{character_name}' before reindexing"
                    )
            except Exception as e:
                if settings.debug:
                    print(
                        f"[RAG CHARACTER INDEXING] Warning: Failed to cleanup existing chunks for '{character_name}': {e}"
                    )
            ...
            if settings.debug:
                print(
                    f"[RAG CHARACTER INDEXING] Error indexing character '{character_name}' in RAG: {e}"
                )

    async def extract_chapter_characters(
        self, chapter_synopsis: str, chapter_num: int, settings: GenerationSettings
    ) -> List[str]:
        ...
        try:
            ...
            if response.json_parsed:
                try:
                    ...
                    if isinstance(parsed_names, list):
                        ...
                        if settings.debug:
                            print(
                                f"[CHARACTER EXTRACTION] Successfully parsed JSON: {character_names}"
                            )
                except (json.JSONDecodeError, AttributeError) as e:
                    if settings.debug:
                        print(
                            f"[CHARACTER EXTRACTION] JSON parsing failed: {e}, falling back to line parsing"
                        )
                    character_names = []
            else:
                if settings.debug:
                    print(
                        f"[CHARACTER EXTRACTION] JSON parsing failed: {response.json_errors}, falling back to line parsing"
                    )
                character_names = []
            ...
            if settings.debug:
                print(
                    f"[CHAPTER CHARACTERS] Error extracting characters for chapter {chapter_num}: {e}"
                )
            return []

    async def fetch_character_sheets_for_chapter(
        self, character_names: List[str], settings: GenerationSettings
    ) -> str:
    ...
    async def update_character_sheets(
        ...
        for character_name in character_names:
            try:
                ...
                if settings.debug:
                    print(
                        f"[CHARACTER UPDATE] Updated sheet for {character_name} based on chapter {chapter_num}"
                    )

            except Exception as e:
                if settings.debug:
                    print(
                        f"[CHARACTER UPDATE] Error updating sheet for {character_name}: {e}"
                    )

    async def extract_chapter_characters_from_outline(
        self, chapter_outline: str, chapter_num: int, settings: GenerationSettings
    ) -> List[str]:
        ...
        try:
            ...
            if response.json_parsed:
                ...
                print(
                    f"[CHARACTER EXTRACTION] Raw response preview: {names_text[:200]}..."
                )
                character_names = []
            ...
            if settings.debug:
                print(
                    f"[CHAPTER CHARACTERS] Error extracting characters from outline for chapter {chapter_num}: {e}"
                )
            return []
    ...
    async def generate_character_summary(
        ...
        if not self.savepoint_manager:
            if settings.debug:
                print(
                    f"[CHARACTER SUMMARY] No savepoint manager available for {character_name}"
                )
            return ""
        ...
        try:
            ...
            current_state_chunk = await self.savepoint_manager.load_step(
                current_state_key
            )

            if (
                not personality_chunk
                and not motivations_chunk
                and not current_state_chunk
            ):
                if settings.debug:
                    print(
                        f"[CHARACTER SUMMARY] No character chunks found for {character_name}"
                    )
                return ""
            ...
            if response and response.content and response.content.strip():
                ...
                if settings.debug:
                    print(
                        f"[CHARACTER SUMMARY] Generated summary for {character_name}: {len(summary)} characters"
                    )
                return summary
            else:
                if settings.debug:
                    print(
                        f"[CHARACTER SUMMARY] Warning: Empty response when generating summary for {character_name}"
                    )
                return ""
            ...
            if settings.debug:
                print(
                    f"[CHARACTER SUMMARY] Error generating summary for {character_name}: {e}"
                )
            return ""
    ...
    async def get_character_summaries(
        ...
        if not self.savepoint_manager:
            if settings.debug:
                print(
                    "[CHARACTER SUMMARIES] No savepoint manager available, returning character names only"
                )
            # Return just the character names as a fallback
            return "\n\n".join(
                [
                    f"**{name}**: Character appears in this scene"
                    for name in character_names
                ]
            )
        ...
        if summaries:
            ...
            if settings.debug:
                print(
                    f"[CHARACTER SUMMARIES] Generated {len(summaries)} character summaries"
                )
            return combined_summaries
        else:
            if settings.debug:
                print(
                    "[CHARACTER SUMMARIES] No character summaries generated, returning character names only"
                )
            # Return just the character names as a fallback
            return "\n\n".join(
                [
    ...
    async def get_character_summaries_list(
        ...
        if not self.savepoint_manager:
            if settings.debug:
                print(
                    "[CHARACTER SUMMARIES LIST] No savepoint manager available, returning character names only"
                )
            # Return just the character names as a fallback
            return "\n\n---\n\n".join(
                [
        ...
        if summaries:
            ...
            if settings.debug:
                print(
                    f"[CHARACTER SUMMARIES LIST] Generated {len(summaries)} character summaries with horizontal rule separators"
                )
            ...
            if settings.debug:
                print(
                    "[CHARACTER SUMMARIES LIST] No character summaries generated, returning character names only"
                )
            # Return just the character names as a fallback
            return "\n\n---\n\n".join(
                [
