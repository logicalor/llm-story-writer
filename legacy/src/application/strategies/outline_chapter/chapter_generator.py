...
class ChapterGenerator:
    ...
    async def generate_chapters(
        self, outline: Outline, settings: GenerationSettings
    ) -> List[Chapter]:
        ...
        try:
            if not self.savepoint_manager:
                ...
                raise StoryGenerationError(
                    "Savepoint manager is required for chapter generation"
                )
            ...
            if chapter_count == 0:
                print(
                    f"No chapter directories found in {story_dir}. Returning empty list."
                )
                return []
            ...
            for chapter_num in range(1, chapter_count + 1):
                try:
                    ...
                    try:
                        if await self.savepoint_manager.has_step(
                            ...
                            try:
                                synopsis = await self.savepoint_manager.load_step(
                                    f"chapter_{chapter_num}/synopsis"
                                )
                        ...
                        print(
                            f"  Error processing outline for Chapter {chapter_num}: {e}"
                        )
                        continue
                    ...
                    try:
                        ...
                        if await self.savepoint_manager.has_step(
                            f"chapter_{chapter_num}/scene_definitions"
                        ):
                            print(
                                f"  Chapter {chapter_num} scene definitions found, loading..."
                            )
                            try:
                                ...
                                try:
                                    ...
                                    if isinstance(scene_definitions, list):
                                        ...
                                        if settings.debug:
                                            print(
                                                f"    Successfully parsed {expected_scene_count} scene definitions from JSON"
                                            )
                                    else:
                                        raise ValueError(
                                            "Expected JSON array of scenes"
                                        )

                                except (json.JSONDecodeError, ValueError) as e:
                                    if settings.debug:
                                        print(
                                            f"    JSON parsing failed: {e}, will regenerate scene definitions"
                                        )
                                    scene_definitions = []
                                    expected_scene_count = 0
                            except Exception as e:
                                if settings.debug:
                                    print(
                                        f"    Error loading scene definitions: {e}, will regenerate"
                                    )
                                scene_definitions = []
                                expected_scene_count = 0
                        ...
                        if expected_scene_count > 0:
                            ...
                            for scene_num in range(1, expected_scene_count + 1):
                                if not await self.savepoint_manager.has_step(
                                    f"chapter_{chapter_num}/scene_{scene_num}"
                                ):
                            ...
                            if all_scenes_exist:
                                print(
                                    f"  Chapter {chapter_num} has all {expected_scene_count} scenes, loading..."
                                )
                                ...
                                for scene_num in range(1, expected_scene_count + 1):
                                    ...
                                    scene_title = await self.savepoint_manager.load_step(
                                        f"chapter_{chapter_num}/scene_{scene_num}_title"
                                    )
                                ...
                                print(
                                    f"  Chapter {chapter_num} missing scenes: {missing_scenes}, regenerating all scenes..."
                                )
                            ...
                            print(
                                f"  Chapter {chapter_num} has no scene definitions, will generate..."
                            )
                        ...
                        if expected_scene_count == 0 or len(chapter_scenes) == 0:
                            ...
                            self.recap_manager.savepoint_manager = (
                                self.savepoint_manager
                            )
                            self.scene_generator.savepoint_manager = (
                                self.savepoint_manager
                            )
                            ...
                            next_chapter_synopsis = (
                                await self._get_next_chapter_synopsis_from_savepoint(
                                    chapter_num, chapter_count, settings
                                )
                            )

                            chapter_outline_for_scene = (
                                await self.savepoint_manager.load_step(
                                    f"chapter_{chapter_num}/disambiguated_outline"
                                )
                            )

                            # Update the scene generator's savepoint manager to ensure character/setting managers have access
                            self.scene_generator.update_savepoint_manager(
                                self.savepoint_manager
                            )
                            ...
                            await self.savepoint_manager.save_step(
                                f"chapter_{chapter_num}/content", chapter_content
                            )
                            print(
                                f"  Chapter {chapter_num} scenes generated and combined."
                            )

                            # Update character and setting sheets based on new chapter content
                            print(
                                f"  Updating character and setting sheets for Chapter {chapter_num}..."
                            )
                            try:
                                # Extract character names from chapter content
                                chapter_characters = await self.character_manager.extract_chapter_characters(
                                    chapter_content, chapter_num, settings
                                )
                                if chapter_characters:
                                    print(
                                        f"    Found characters in chapter: {', '.join(chapter_characters)}"
                                    )
                                    # Update character sheets based on new chapter content
                                    await (
                                        self.character_manager.update_character_sheets(
                                            chapter_characters,
                                            chapter_content,
                                            chapter_num,
                                            settings,
                                ...
                                chapter_settings = (
                                    await self.setting_manager.extract_chapter_settings(
                                        chapter_content, chapter_num, settings
                                    )
                                )
                                if chapter_settings:
                                    print(
                                        f"    Found settings in chapter: {', '.join(chapter_settings)}"
                                    )
                                ...
                                print(
                                    f"    Character and setting sheets updated for Chapter {chapter_num}"
                                )

                            except Exception as e:
                                print(
                                    f"    Warning: Failed to update character/setting sheets for Chapter {chapter_num}: {e}"
                                )
                                # Continue processing even if updates fail
                    except Exception as e:
                        print(
                            f"  Error processing scenes for Chapter {chapter_num}: {e}"
                        )
                        continue
                    ...
                    try:
                        if await self.savepoint_manager.has_step(
                            ...
                            print(
                                f"  Chapter {chapter_num} recap already exists, skipping..."
                            )
                        else:
                            ...
                            chapter_recap = (
                                await self.recap_manager.generate_chapter_recap(
                                    chapter_num=chapter_num,
                            ...
                            await self.savepoint_manager.save_step(
                                f"chapter_{chapter_num}/recap", chapter_recap
                            )

                            if settings.debug:
                                print(
                                    f"    Generated and saved recap for chapter {chapter_num}"
                                )
                            else:
                                print(
                                    f"    Chapter {chapter_num} recap generated and saved."
                                )
                    except Exception as e:
                        print(
                            f"  Error processing recap for Chapter {chapter_num}: {e}"
                        )
                        # Continue to next step even if recap fails

                    # Step 4: Generate title and create Chapter object
                    print(
                        f"  Step 4: Creating Chapter object for Chapter {chapter_num}..."
                    )
                    try:
    ...
    async def _generate_chapter_outline(
        ...
        chapter_characters = await self.character_manager.extract_chapter_characters(
            chapter_synopsis, chapter_num, settings
        )

        # Fetch character sheets for this chapter
        character_sheets = (
            await self.character_manager.fetch_character_sheets_for_chapter(
                chapter_characters, settings
            )
        )

        # Extract setting names for this chapter
        chapter_settings = await self.setting_manager.extract_chapter_settings(
            chapter_synopsis, chapter_num, settings
        )

        # Fetch setting sheets for this chapter
        setting_sheets = await self.setting_manager.fetch_setting_sheets_for_chapter(
            chapter_settings, settings
        )
    ...
    async def _generate_chapter_outline_impl(
        ...
        validation_issues = await self._validate_outline_quality(
            core_outline, chapter_num, settings
        )
        ...
        # Run disambiguator
        disambiguated_outline = await self._run_disambiguator(
            core_outline, chapter_num, settings
        )

        # Run cleanup
        final_outline = await self._run_cleanup(
            disambiguated_outline, chapter_num, settings
        )
        ...
        if self.savepoint_manager:
            await self.savepoint_manager.save_step(
                f"chapter_{chapter_num}/outline", formatted_outline
            )
    ...
    async def _validate_outline_quality(
        self, outline: str, chapter_num: int, settings: GenerationSettings
    ) -> List[str]:
    ...
    async def _format_outline_structure(
        self, outline: str, settings: GenerationSettings
    ) -> str:
    ...
    async def _get_chapter_outline(
        self, chapter_num: int, outline: str, settings: GenerationSettings
    ) -> str:
        ...
        response = await execute_prompt_with_savepoint(
            ...
            variables={"chapter_num": chapter_num, "outline": outline},
    ...
    async def _get_next_chapter_outline(
        ...
        return await self._get_chapter_outline(
            chapter_num + 1, outline.story_elements, settings
        )

    async def _get_next_chapter_synopsis_from_savepoint(
        self, chapter_num: int, total_chapters: int, settings: GenerationSettings
    ) -> str:
    ...
    async def _generate_chapter_content(
        ...
        model_config = ModelConfig.from_string(
            self.config["models"]["chapter_stage1_writer"]
        )
    ...
    async def _generate_chapter_title(
        self, chapter_num: int, content: str, outline: str, settings: GenerationSettings
    ) -> str:
    ...
    async def generate_chapter_synopses(
        ...
        if settings.debug:
            print(
                "[CHAPTER SYNOPSES] Generating synopses for chapters from combined outline"
            )
        ...
        for chapter_data in chapter_list:
            ...
            if settings.debug:
                print(
                    f"[CHAPTER SYNOPSES] Generating synopsis for Chapter {chapter_num}: {chapter_title}"
                )
            ...
            if self.savepoint_manager:
                await self.savepoint_manager.save_step(
                    f"chapter_{chapter_num}/synopsis", synopsis
                )

        if settings.debug:
            print(
                f"[CHAPTER SYNOPSES] Completed generating {len(synopses)} chapter synopses"
            )
    ...
    async def _extract_chapters_from_outline(
        ...
        if response.json_parsed:
            ...
            try:
                ...
                if settings.debug:
                    print(
                        f"[CHAPTER SYNOPSES] Successfully parsed {len(chapter_list)} chapters from JSON"
                    )
                return chapter_list
            ...
            if settings.debug:
                ...
                print(
                    f"[CHAPTER SYNOPSES] Raw response preview: {response.content[:200]}..."
                )
    ...
    async def _generate_single_chapter_synopsis(
        ...
        characters = await self.character_manager.extract_character_names(
            story_elements, settings
        )
        character_summaries = await self.character_manager.get_character_summaries(
            characters, settings
        )
        ...
        setting_names = await self.setting_manager.extract_setting_names(
            story_elements, settings
        )
        setting_summaries = await self.setting_manager.get_setting_summaries(
    ...
    def _fallback_chapter_extraction(
    ...
    async def generate_chunked_outline(
        self, story_elements: str, base_context: str, settings: GenerationSettings
    ) -> str:
        """Generate chunked outline by processing chapters in manageable chunks."""
        if settings.debug:
            print(
                f"[CHUNKED OUTLINE] Generating chunked outline for {settings.wanted_chapters} chapters"
            )
        ...
        if settings.debug:
            print(
                f"[CHUNKED OUTLINE] Using chunk size {chunk_size}, total chunks: {total_chunks}"
            )
        ...
        for chunk_num in range(total_chunks):
            ...
            if settings.debug:
                print(
                    f"[CHUNKED OUTLINE] Processing chunk {chunk_num + 1}/{total_chunks}: chapters {chunk_start}-{chunk_end}"
                )
    ...
    async def _analyze_chunk_continuity(
        self, chunk_outline: str, previous_chunks: str, settings: GenerationSettings
    ) -> str:
    ...
    async def plan_next_chapter_progressive(
        ...
        try:
            if (
                not hasattr(self, "story_state_manager")
                or not self.story_state_manager.story_context
            ):
                raise StoryGenerationError(
                    "Story context must be initialized before progressive planning"
                )
            ...
            if settings.debug:
                print(
                    f"[PROGRESSIVE PLANNING] Chapter {chapter_state.chapter_number} planning coordinated"
                )
            ...
            raise StoryGenerationError(
                f"Failed to coordinate progressive chapter planning: {e}"
            ) from e
    ...
    async def revise_outline_progressive(
        ...
        try:
            if settings.debug:
                print(
                    f"[PROGRESSIVE PLANNING] Coordinating outline revision for chapter {chapter_num}..."
                )

            # Delegate revision to StoryStateManager
            chapter_state = await self.story_state_manager.revise_chapter_plan(
                chapter_num, settings
            )

            if settings.debug:
                print(
                    f"[PROGRESSIVE PLANNING] Chapter {chapter_num} revision coordinated"
                )
            ...
            raise StoryGenerationError(
                f"Failed to coordinate outline revision: {e}"
            ) from e
