...
class RAGService:
    ...
    async def index_content(
        ...
        try:
            ...
            logger.debug(
                f"Generated embedding type: {type(embedding)}, length: {len(embedding) if isinstance(embedding, list) else 'N/A'}"
            )
            ...
            if hasattr(embedding, "tolist"):
                ...
                logger.debug(
                    f"Converted numpy array to list, new type: {type(embedding)}"
                )
    ...
    async def search_similar_reranked(
        ...
        try:
            ...
            raw_results = await self.search_similar(
                ...
                limit=limit * 2
                if limit
                else self.max_context_chunks * 2,  # Get more candidates
            ...
            if self.use_reranker and self.reranker:
                logger.debug(
                    f"Reranking {len(raw_results)} results using {rerank_strategy} strategy with {self.reranker_type} reranker"
                )
                ...
                if self.reranker_type == "model_based":
                    ...
                    reranked_results = await self.reranker.rerank_results(
                        query=query, results=raw_results, strategy=rerank_strategy
                    )
    ...
    async def _get_character_context(self, story_id: int, chapter_number: int) -> str:
        ...
        try:
            ...
            if not character_chunks:
                ...
                character_chunks = await self.vector_store.get_story_content(
                    story_id=story_id, content_type="character"
                )
    ...
    async def _get_setting_context(self, story_id: int, chapter_number: int) -> str:
        ...
        try:
            ...
            setting_chunks = await self.vector_store.get_story_content(
                story_id=story_id, content_type="setting", chapter_number=chapter_number
            )

            if not setting_chunks:
                # Fall back to all setting information
                setting_chunks = await self.vector_store.get_story_content(
                    story_id=story_id, content_type="setting"
                )
    ...
    async def _get_plot_context(self, story_id: int, chapter_number: int) -> str:
        ...
        try:
            ...
            plot_chunks = await self.vector_store.get_story_content(
                story_id=story_id, content_type="outline"
            )
    ...
    async def _get_event_context(self, story_id: int, chapter_number: int) -> str:
        ...
        try:
            ...
            event_chunks = await self.vector_store.get_story_content(
                story_id=story_id, content_type="event"
            )
    ...
    async def delete_content_by_type_and_metadata(
        ...
        try:
            ...
            logger.info(
                f"Deleted {deleted_count} {content_type} chunks for story {story_id}"
            )
