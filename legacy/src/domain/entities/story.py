...
class Outline:
    ...
    initial_outline: Optional[str] = (
        None  # The original outline before critique refinement
    )
    ...
    def __post_init__(self):
        ...
        if self.word_count < 0:
            raise ValidationError(
                f"Word count cannot be negative, got {self.word_count}"
            )

        if self.chapter_count < 0:
            raise ValidationError(
                f"Chapter count cannot be negative, got {self.chapter_count}"
            )
...
class Story:
    ...
    def __post_init__(self):
        ...
        if chapter_numbers != expected_numbers:
            raise ValidationError(
                f"Chapter numbers must be sequential starting from 1, got {chapter_numbers}"
            )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Story":
        ...
        for chapter_data in data["chapters"]:
            ...
            chapter_without_scenes = {
                k: v for k, v in chapter_data.items() if k != "scenes"
            }
