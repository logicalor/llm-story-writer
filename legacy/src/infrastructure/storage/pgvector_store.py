...
class PgVectorStore:
    ...
    async def _register_vector_type(self):
        ...
        try:
            ...
            try:
                async with self._pool.acquire() as conn:
                    ...
                    logger.info(
                        "Vector type handler registered successfully on connection"
                    )
            except Exception as conn_e:
                logger.warning(
                    f"Failed to register vector type handler on connection: {conn_e}"
                )
        ...
        try:
            ...
            logger.warning(
                f"Failed to manually register vector type handler: {manual_e}"
            )
    ...
    async def _ensure_tables_exist(self):
        ...
        async with self._pool.acquire() as conn:
            ...
            if not result:
                raise StorageError(
                    "Stories table not found. Please run init.sql first."
                )
    ...
    async def create_story(self, story_name: str, prompt_file_name: str) -> int:
        ...
        async with self._pool.acquire() as conn:
            try:
                ...
                story_id = await conn.fetchval(
                    "SELECT id FROM stories WHERE story_name = $1", story_name
                )
    ...
    async def store_embedding(
        ...
        async with self._pool.acquire() as conn:
            try:
                ...
                logger.debug(
                    f"store_embedding received embedding type: {type(embedding)}, length: {len(embedding) if isinstance(embedding, list) else 'N/A'}"
                )
                logger.debug(
                    f"store_embedding embedding preview: {str(embedding)[:100]}..."
                )
                ...
                logger.debug(
                    f"Stored embedding for content type '{content_type}' with ID {chunk_id}"
                )
                return chunk_id
            except Exception as e:
        ...
        async with self._pool.acquire() as conn:
            try:
                ...
                if story_id:
                    ...
                    query_parts.append(
                        "WHERE 1=1"
                    )  # Always true condition for cross-story search
    ...
    async def get_story_content(
        ...
        async with self._pool.acquire() as conn:
            try:
                query_parts = ["SELECT id, content_type, content, metadata"]
                query_parts.append("FROM content_chunks")
                ...
                for row in rows:
                    metadata = json.loads(row["metadata"]) if row["metadata"] else {}
                    results.append(
                        (row["id"], row["content_type"], row["content"], metadata)
                    )
    ...
    async def delete_content_by_type_and_metadata(
        ...
        async with self._pool.acquire() as conn:
            try:
                ...
                if metadata_filters:
                    for key, value in metadata_filters.items():
                        ...
                        count_query_parts.append(
                            f"AND metadata->>'{key}' = ${param_count}"
                        )
                ...
                logger.info(
                    f"Deleted {chunk_count} {content_type} chunks for story {story_id}"
                )
                return chunk_count

            except Exception as e:
                logger.error(
                    f"Failed to delete {content_type} content for story {story_id}: {e}"
                )
                raise StorageError(
                    f"Failed to delete {content_type} content for story {story_id}: {e}"
                )
    ...
    async def delete_story_content(self, story_id: int) -> int:
        ...
        async with self._pool.acquire() as conn:
            try:
                ...
                logger.info(
                    f"Deleted {deleted_count} content chunks for story {story_id}"
                )
