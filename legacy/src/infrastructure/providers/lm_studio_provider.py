...
class LMStudioProvider(ModelProvider):
    ...
    async def stream_text(
        ...
        try:
            ...
            # Prepare the request payload
            payload = {"messages": messages, "stream": True, **options}
    ...
    async def download_model(self, model_config: ModelConfig) -> None:
        ...
        print(
            f"Please download the model '{model_config.name}' through the LM Studio application."
        )
    ...
    def _prepare_options(
        ...
        # If max_tokens is specified but exceeds infrastructure limit, cap it
            ...
            print(
                f"[LM STUDIO] Warning: Requested max_tokens {options['max_tokens']} exceeds infrastructure limit {self.context_length}, capping to {self.context_length}"
            )
            options["max_tokens"] = self.context_length
        ...
        if seed is not None:
            ...
            if should_randomize:
                ...
                print(
                    f"[LM STUDIO] Seed randomized: {seed} + {random_offset} = {options['seed']}"
                )
            else:
        ...
        if "max_tokens" in options:
            print(
                f"[LM STUDIO] Context length: {options['max_tokens']} (from model config)"
            )
        else:
            print(
                f"[LM STUDIO] Context length: {options['max_tokens']} (from infrastructure default)"
            )
    ...
    async def generate_multistep_conversation(
        ...
        try:
            ...
            raise ModelProviderError(
                f"LM Studio multi-step conversation failed: {e}"
            ) from e
    ...
    async def _generate_multistep_conversation_non_streaming(
        ...
        try:
            ...
            for i, user_message in enumerate(user_messages, 1):
                ...
                # Prepare the request payload
                payload = {"messages": conversation_messages, **options}
    ...
    async def _generate_multistep_conversation_with_streaming(
        ...
        try:
            ...
            raise ModelProviderError(
                f"LM Studio multi-step conversation streaming failed: {e}"
            ) from e
