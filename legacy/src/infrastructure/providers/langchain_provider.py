...
class LangChainProvider(ModelProvider):
    ...
    async def generate_multistep_conversation(
        ...
        try:
            ...
            raise ModelProviderError(
                f"LangChain multi-step conversation failed: {e}"
            ) from e
    ...
    async def _generate_multistep_conversation_with_streaming(
        ...
        try:
            ...
            raise ModelProviderError(
                f"LangChain multi-step conversation streaming failed: {e}"
            ) from e
    ...
    async def download_model(self, model_config: ModelConfig) -> None:
        ...
        print(
            f"Please ensure the model '{model_config.name}' is available through the configured provider."
        )
    ...
    async def _create_langchain_llm(self, model_config: ModelConfig):
        ...
        if provider == "openai":
            ...
            return ChatOpenAI(model=model_name, openai_api_key=api_key, **parameters)

        elif provider == "anthropic":
            from langchain_anthropic import ChatAnthropic

            return ChatAnthropic(
                model=model_name, anthropic_api_key=api_key, **parameters
            )
            ...
            return HuggingFaceEndpoint(
                repo_id=model_name, huggingfacehub_api_token=api_key, **parameters
            )
    ...
    def _prepare_options(
        ...
        # If max_tokens is specified but exceeds infrastructure limit, cap it
            ...
            print(
                f"[LANGCHAIN] Warning: Requested max_tokens {options['max_tokens']} exceeds infrastructure limit {self.context_length}, capping to {self.context_length}"
            )
            options["max_tokens"] = self.context_length
        ...
        if seed is not None:
            ...
            if should_randomize:
                ...
                print(
                    f"[LANGCHAIN] Seed randomized: {seed} + {random_offset} = {options['seed']}"
                )
            else:
        ...
        if "max_tokens" in options:
            print(
                f"[LANGCHAIN] Context length: {options['max_tokens']} (from model config)"
            )
        else:
            print(
                f"[LANGCHAIN] Context length: {options['max_tokens']} (from infrastructure default)"
            )
    ...
    def _create_conversation_memory(self):
        ...
        try:
            ...
            memory = ConversationBufferMemory(
                memory_key="history", return_messages=True, input_key="input"
            )
