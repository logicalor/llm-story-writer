...
class OllamaProvider(ModelProvider):
    ...
    def _estimate_token_count(self, messages: List[Dict[str, str]]) -> int:
        ...
        message_overhead = (
            len(messages) * 10
        )  # ~10 tokens per message for role/structure
    ...
    def _log_prompt_stats(
        ...
        if token_count > context_length * 0.8:  # Warn at 80% of context length
            print(
                f"[PROMPT STATS] ⚠️  WARNING: Prompt uses {token_count:,} tokens, approaching context limit of {context_length:,}"
            )
        elif token_count > context_length * 0.6:  # Info at 60% of context length
            print(
                f"[PROMPT STATS] ℹ️  INFO: Prompt uses {token_count:,} tokens ({token_count / context_length * 100:.1f}% of {context_length:,} context limit)"
            )
    ...
    async def _generate_text_non_streaming(
        ...
        try:
            ...
            response = await asyncio.to_thread(
                client.chat, model=model_config.name, messages=messages, options=options
            )
    ...
    async def stream_text(
        ...
        try:
            ...
            for chunk in stream:
                ...
                if (
                    "message" in chunk
                    and "thinking" in chunk["message"]
                    and chunk["message"]["thinking"]
                ):
    ...
    async def download_model(self, model_config: ModelConfig) -> None:
        ...
        try:
            ...
            stream = await asyncio.to_thread(
                client.pull, model_config.name, stream=True
            )
            ...
            for chunk in stream:
                if "completed" in chunk and "total" in chunk:
                    ...
                    print(
                        f"Downloading {model_config.name}: {progress * 100:.2f}% ({completed_size:.3f}GB/{total_size:.3f}GB)",
                        end="\r",
                    )
                else:
                    print(
                        f"{chunk.get('status', 'Unknown')} {model_config.name}",
                        end="\r",
                    )
            ...
            raise ModelProviderError(
                f"Failed to download model {model_config.name}: {e}"
            ) from e
    ...
    def _prepare_options(
        ...
        # If num_ctx is specified but exceeds infrastructure limit, cap it
            ...
            print(
                f"[OLLAMA] Warning: Requested num_ctx {options['num_ctx']} exceeds infrastructure limit {self.context_length}, capping to {self.context_length}"
            )
            options["num_ctx"] = self.context_length
        ...
        if seed is not None:
            ...
            if should_randomize:
                ...
                print(
                    f"[OLLAMA] Seed randomized: {seed} + {random_offset} = {options['seed']}"
                )
            else:
        ...
        if any(
            thinking_model in model_config.name.lower()
        ...
        if "num_ctx" in options:
            ...
            print(
                f"[OLLAMA] Context length: {options['num_ctx']} (from infrastructure default)"
            )
    ...
    async def generate_multistep_conversation(
        ...
        try:
            ...
            raise ModelProviderError(
                f"Ollama multi-step conversation failed: {e}"
            ) from e
    ...
    async def _generate_multistep_conversation_non_streaming(
        ...
        try:
            ...
            for i, user_message in enumerate(user_messages, 1):
                if debug:
                    print(
                        f"[CONVERSATION] Step {i}/{len(user_messages)}: Processing user message"
                    )
                    print(
                ...
                if debug:
                    print(
                        f"[CONVERSATION] Step {i} Response: {response_text[:100]}{'...' if len(response_text) > 100 else ''}"
                    )
    ...
    async def _generate_multistep_conversation_with_streaming(
        ...
        try:
            ...
            for i, user_message in enumerate(user_messages, 1):
                ...
                response_text = ""
                async for chunk in self.stream_text(
                    messages=conversation_messages, model_config=model_config, seed=seed
                ):
                    print(chunk, end="", flush=True)
                    response_text += chunk
                ...
                if debug:
                    print(
                        f"[CONVERSATION] Step {i} Response Length: {len(response_text)} characters"
                    )
            ...
            raise ModelProviderError(
                f"Ollama multi-step conversation streaming failed: {e}"
            ) from e
