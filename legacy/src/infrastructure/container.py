...
class Container(containers.DeclarativeContainer):
    ...
    logger = providers.Singleton(
        StructuredLogger,
        log_file=providers.Callable(
            lambda logs_dir: Path(logs_dir) / "app.log", config.logs_dir
        ),
        level=LogLevel.INFO,
        enable_console=True,
    )

    prompt_loader = providers.Singleton(PromptLoader, prompts_dir="src/prompts")
    ...
    file_storage = providers.Singleton(
        ...
        base_path=providers.Callable(
            lambda output_dir: Path(output_dir), config.output_dir
        ),
    )
    ...
    savepoint_repository = providers.Singleton(
        ...
        base_path=providers.Callable(
            lambda savepoint_dir: Path(savepoint_dir), config.savepoint_dir
        ),
    )
    ...
    ollama_embedding_provider = providers.Singleton(
        ...
        model=providers.Callable(
            lambda model: (
                model.replace("ollama://", "") if model else "nomic-embed-text"
            ),
    ...
    pgvector_store = providers.Factory(
        ...
        connection_string=providers.Callable(
            lambda host, database, user, password: (
                f"postgresql://{user}:{password}@{host.split(':')[0]}:5432/{database}"
            ),
    ...
    strategy = providers.Factory(
        lambda factory, model_provider, config, savepoint_repo, rag_service: (
            factory.create_strategy_with_prompts(
                config.get("generation", {}).get("strategy", "outline-chapter"),
                model_provider,
                config,
                savepoint_repo,
                rag_service,
