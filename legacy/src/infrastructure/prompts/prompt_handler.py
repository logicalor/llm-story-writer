...
class PromptHandler:
    ...
    async def execute_prompt(self, request: PromptRequest) -> PromptResponse:
                    ...
                    if request.expect_json and request.json_schema:
                        try:
                            ...
                            if parsed_content is not None:
                                ...
                                if request.debug:
                                    print(
                                        f"[JSON PARSING] Successfully parsed cached JSON response for {request.prompt_id}"
                                    )
                            else:
                                json_errors = (
                                    "Failed to extract valid JSON from cached response"
                                )
                                if request.debug:
                                    print(
                                        f"[JSON PARSING] No valid JSON found in cached response for {request.prompt_id}"
                                    )
                                    print(
                                        f"[JSON PARSING] Raw cached content preview: {cached_content[:200]}..."
                                    )
                            ...
                            if request.debug:
                                print(
                                    f"[JSON PARSING] Failed to parse cached JSON response for {request.prompt_id}: {e}"
                                )
        ...
        try:
            ...
            raise StoryGenerationError(
                f"Failed to load prompt '{request.prompt_id}': {e}"
            )
        ...
        messages = self._prepare_messages(
            prompt_content, request.prepend_message, request.system_message
        )
        ...
        try:
            if request.model_config is None:
                raise StoryGenerationError(
                    "Model configuration is required for prompt execution"
                )
            ...
            if request.stream:
                ...
                thinking_content, content = self._extract_thinking_and_content(
                    raw_response
                )
            else:
            ...
            raise StoryGenerationError(
                f"Failed to execute prompt '{request.prompt_id}': {e}"
            )
        ...
        if request.savepoint_id and self.savepoint_repo:
            try:
                ...
                await self.savepoint_repo.save_savepoint(
                    request.savepoint_id, savepoint_data
                )
            except Exception as e:
                # Log the error but don't fail the request
                print(
                    f"Warning: Failed to save savepoint '{request.savepoint_id}': {e}"
                )
        ...
        if request.expect_json and request.json_schema:
            try:
                ...
                if parsed_content is not None:
                    ...
                    if request.debug:
                        print(
                            f"[JSON PARSING] Successfully parsed JSON response for {request.prompt_id}"
                        )
                else:
                    json_errors = "Failed to extract valid JSON from response"
                    if request.debug:
                        print(
                            f"[JSON PARSING] No valid JSON found in response for {request.prompt_id}"
                        )
                ...
                if request.debug:
                    print(
                        f"[JSON PARSING] Failed to parse JSON response for {request.prompt_id}: {e}"
                    )
        ...
        return PromptResponse(
            ...
            savepoint_id=request.savepoint_id,
            was_cached=False,
            model_used=request.model_config.to_string()
            if request.model_config
            else None,
            execution_time=execution_time,
            json_parsed=json_parsed,
    ...
    async def execute_json_prompt(
        self, request: PromptRequest, required_attributes: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Extract thinking and content
            thinking_content, content = self._extract_thinking_and_json_content(
                raw_response
            )
        except Exception as e:
            raise StoryGenerationError(
                f"Failed to execute JSON prompt '{request.prompt_id}': {e}"
            )
    ...
    def _extract_thinking_and_json_content(
    ...
    def _create_savepoint_data(
        ...
        savepoint_data = {"_frontmatter": frontmatter, "_body": content}
