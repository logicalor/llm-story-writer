...
| `rag-query` | Retrieve prior chapter voice/context examples | `operation`: `index`\|`query`, `name`, `query`, `contentType`, `chapterNum`, `nResults` |
...
3. For each chapter number `N` in `chapter_numbers`:
   ...
   - Query prior context via `rag-query` with `operation: "query"`, `name: story_name`, `contentType: "chapter"`, `chapterNum: N`, `query: "voice consistency, pacing, character voice, tone, and continuity for chapters before chapter {N}"`, `nResults: 5`.
