...
Use the `scrub_model` role from story config for the analysis call when configured. If `config.models.scrub_model` is absent, fall back to the default model.
...
6. For each actionable issue, call `scene-writer` with `operation: "revise"`, `name: story_name`, `chapterNum: N`, `sceneContent: current chapter text`, and `feedback` set to a surgical correction instruction derived from `type`, `original_text`, `suggested_replacement`, and `line_context`. Stop after 5 revision calls for the chapter. After each successful revision, replace the in-memory chapter text with the returned text.
