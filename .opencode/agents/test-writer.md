...
**Mock return type schema fidelity:** When mocking an agent's return value in orchestrator or integration tests, the mock dict must match the actual return type's field value *types* — not just field names. For example, if the real agent returns `{"events": ""}` (an empty string), mocking it as `{"events": []}` (an empty list) produces a type mismatch: downstream code that calls `.split()` or uses the value as a string will raise `AttributeError` or `TypeError`, while tests that only verify pipeline completion (not field values) continue to pass. Pattern: before writing the mock, read the agent's `run()` return statement to confirm the type of each field. Pay particular attention to fields that could be either string or list depending on their purpose — LLM agents returning event summaries or prose fragments return strings, not lists. (Source: issue #346, PR #347 — `events: []` mock produced a type mismatch where the real agent returns `events: ""`.)
...
**Interrupt+resume integration test isolation:** When writing integration tests that verify pipeline interrupt+resume behaviour (`resume_pipeline()`), use a `contextmanager` that bundles all agent patches and `STORIES_DIR` patches together, and use **different story names** (e.g. `"baseline-story"`, `"resume-story"`) within the same `tmp_path` — shared names cause savepoint collisions between runs. Write chapter disk files (e.g. `chapters/chapter_1.md`) **before** calling `_write_savepoint` — the resume logic reads from disk, so the file must exist at the point the savepoint is written. Use `AsyncMock` providers and patch agent classes directly rather than patching `provider.generate_text` slots, to avoid advisory-phase mock slot consumption (see advisory phases note above). `_write_savepoint` is directly importable from `presentation.orchestrator` as a test hook for pre-writing partial pipeline state. (Source: issue #323, PR #335.)
