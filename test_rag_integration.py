...
async def test_rag_integration():
    ...
    try:
        ...
        print(
            f"   - Outline generator RAG service: {strategy_no_rag.outline_generator.rag_service}"
        )
        ...
        class MockRAGService:
            ...
            async def create_story(self, story_name: str, prompt_file_path: str) -> int:
                ...
                print(
                    f"      📝 Mock RAG: Created story {story_id} for '{prompt_file_path}'"
                )
                return story_id
        ...
        strategy_with_rag = OutlineChapterStrategy(
            model_provider=None,  # Mock for testing
            config={"max_chunk_size": 1000, "overlap_size": 200},
            prompt_loader=None,  # Mock for testing
            savepoint_repo=None,  # Mock for testing
        ...
        print(
            f"   - Outline generator RAG service: {strategy_with_rag.outline_generator.rag_service}"
        )
        print(
            f"   - Outline generator RAG integration: {strategy_with_rag.outline_generator.rag_integration}"
        )
        ...
        print(
            f"   - Current prompt filename: {strategy_with_rag.get_current_prompt_filename()}"
        )
