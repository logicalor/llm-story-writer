...
When you already have an issue number (e.g., the user says "work on issue #42"), use `github_issue_read` — **do not search for it**.
...
If you have an issue number, **do not use `github_search_issues`**. All of these are wrong:
...
Use `github_search_issues` when you need to find issues by keyword, label, or state. The search is **already scoped to `is:issue`** — do not add `is:issue` to your query.
...
1. **Searching by issue number** — `number:` is NOT a valid GitHub search qualifier. Queries like `number:42`, `is:issue number:42`, or `#42` will fail or return wrong results. If you have the issue number, use `github_issue_read` with `method: "get"` instead.
...
Use `github_list_issues` to get a paginated list of issues, optionally filtered by state and labels. This is useful for browsing all open issues or issues with a specific label.
...
Use `github_issue_write` with `method: "create"`. There is **no** `github_create_issue` tool.
...
Use `github_issue_write` with `method: "update"`. You **must** provide `issue_number`.
...
Use `github_add_issue_comment`. This also works for pull requests (PRs are issues in GitHub's API).
