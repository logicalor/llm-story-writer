...
**No live LLM in unit tests:** Unit tests must never call LM Studio, Ollama, OpenAI-compatible HTTP endpoints, or any real model provider. Before writing any test that touches LLM-facing code, identify the existing seam and use the project-standard mock strategy:

- **Agent/provider path:** inject a `MagicMock`/`AsyncMock` provider and stub `provider.stream_text` or `provider.generate_text`. Do not instantiate a real OpenAI-compatible provider in `tests/unit/`.
- **Tool path:** patch the tool module's local `_call_llm`, `_call_llm_messages`, or `_chat_completion` shim, not the distant HTTP layer, unless the test is specifically for `tools._llm`.
- **Recap context rendering:** if a mocked `assemble_context` returns non-empty `recap_snippets`, also patch the consumer module's imported `render_recap_as_markdown` (for example `patch("presentation.agents.chapter_outline_expander.render_recap_as_markdown", return_value="...")`). `render_recap_as_markdown()` calls `tools._llm.generate_text()` internally.
- **Subprocess tool tests:** parent-process monkeypatches do not cross into the child process. Set `LLM_API_BASE` to an unreachable URL in the subprocess `env` (use `http://127.0.0.1:1`) and only exercise LLM operations through explicit data escape hatches or error paths unless the child process has its own in-test mock mechanism.
- **Unit-test guard:** `tests/unit/conftest.py` blocks known LLM entrypoints and sets `LLM_API_BASE=http://127.0.0.1:1`. If this fixture fails a new test, fix the test by mocking the correct local seam; do not weaken or bypass the guard. Only `tests/unit/test_llm_client.py` may exercise `tools._llm` directly, and it must mock `requests.post`.
- **Live tests:** tests that intentionally require a live LLM belong in `tests/integration/` and must use the suite-standard `llm_available` fixture.
...
**New pipeline phase breaks existing orchestrator tests:** When a new pipeline phase (agent class) is inserted before existing phases, every test that mocks `provider.generate_text` with a `side_effect=[...]` list silently consumes one extra slot — causing downstream phase assertions to receive the wrong response or raise `StopIteration`. Fix by patching the new agent **class** itself (`patch("src.presentation.orchestrator.NewAgent")`) rather than extending the `side_effect` list. This prevents the phase from consuming any mock slots and requires no knowledge of call order. See `gotchas.md` #040. **Context manager alias rule:** When using a multi-target `with (patch(...) as alias, ...)` block, confirm every `alias.return_value = ...` line is inside the `with` block — using the alias outside the block raises `NameError`.

**Advisory pipeline phases and mock slot consumption:** Advisory phases (wrapped in `try/except Exception`) consume mock slots conditionally based on provider type. With `provider = MagicMock()`: no patch needed — the advisory guard absorbs the `TypeError` from awaiting a non-coroutine mock. With `provider.generate_text = AsyncMock(side_effect=[...])`: must patch all advisory agent classes — the mock call succeeds and consumes slots, shifting downstream responses. `StoryMetadataAgent` runs as 3 advisory phases making 3 LLM calls each (up to 9 slots consumed). Patch it as `patch("presentation.orchestrator.StoryMetadataAgent")` and set `metadata_cls.return_value.run = AsyncMock(return_value=_story_metadata_result())`. See `gotchas.md` #045.
