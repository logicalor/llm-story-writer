...
- [ ] **Typed SDK type-check: prefer `cast()` over `# type: ignore`** — when a PR integrates a typed Python SDK with complex generics (e.g., OpenAI SDK 2.x union return types) and mypy cannot narrow the type, verify the fix uses `cast(TargetType, expr)` rather than `# type: ignore`. `cast()` documents intent, preserves downstream type safety, and has no runtime cost. `# type: ignore` suppresses all checking for the line and allows future regressions to pass silently. `isinstance()` is appropriate only when the type is genuinely uncertain (e.g., after `json.loads()` on untrusted input), not when the SDK contractually guarantees it. (Source: issue #211, PR #217 — `OpenAIAsyncProvider` added six `cast()` calls for OpenAI SDK types; see `gotchas.md` #035.)
...
- [ ] **Tool call contracts** — for any new or modified agent instruction file that includes tool invocations, verify each call against the tool source before accepting: (a) operation names match the Python CLI dispatch (grep `src/tools/*.py` for valid operation values), (b) parameter key names match the exact argument names the Python tool expects — mismatched keys are silently ignored or produce wrong output; (c) any omitted parameter is confirmed safe — check the tool's default value and verify it produces correct behaviour in this context (e.g. `mode` defaults to `"outline"` in `critique-runner`; a chapter-context agent that omits `mode` will evaluate the wrong artifact type without erroring); (d) any parameter that looks optional in the agent-facing schema — verify in the Python source that the field is genuinely optional for the active `operation`; Python tools use conditional validation (`if args.operation == "revise" and not args.scene_num: sys.exit(2)`) that may not be reflected in the agent schema; (e) any parameter passed to the tool is verified to be consumed by the Python backend for the active operation — verify by checking `argparse.add_argument` declarations and the operation dispatch branch in `src/tools/*.py`
...
- [ ] **Prompt variable names override issue description stage ordering** — when an issue description or task spec states a pipeline stage order but the prompt templates use `{variable_name}` placeholders whose names conflict with that order (e.g. a prompt labelled "enrich" accepts `{timed_events}` as input, implying timing must run first), the prompt variables define the correct order — the issue description is wrong. During review, for each stage in a new or modified multi-stage pipeline, read the prompt file and confirm its `{input_variable}` names are produced by a preceding stage in the wired order. Do not rely on the issue description alone. (Source: issue #297, PR #309 — issue described enrich-before-timing but `{timed_events}` in the enrich prompt required timing-first; Coder trusted the prompt correctly but the conflict was not caught in review.)
...
- [ ] **Agent call site parameter casing matches tool schema exactly** — parameter key names are case-sensitive. When an agent or skill instruction file describes a tool call (`Call wiki-extract (operation: ..., chapterNumber: N, ...)`), the parameter keys in the call body must match the exact argument names the Python tool expects character-for-character. A snake_case rendering of the same word (`chapter_number`) is a distinct key and produces a runtime failure, with the entire workflow becoming non-functional. Do not confuse this with the issue #22 batch-payload convention — direct tool call params use camelCase (matching the OpenCode schema); JSON keys *inside* a `payload` string use snake_case. (Source: issue #144, PR #145 — `wiki-maintainer.md` Mode 2 used `chapter_number` and `chapter_text_path` while the schema declared `chapterNumber` and `chapterTextPath`; flagged unanimously.)
...
- [ ] **Dual critic-type list parity** — if the PR adds, renames, or removes an `outline_review/` prompt file (a critic persona), verify that `OUTLINE_CRITIC_TYPES` is updated in **both** `src/presentation/agents/outline_critic.py` (Python orchestrator path) **and** `src/tools/critique_runner.py` (LLM orchestrator path via CLI). The two lists cannot share a single import because the CLI tool cannot cleanly import from the presentation layer; silent drift causes the two orchestration paths to evaluate different critic sets with no error. (Source: issue #300, PR #312.)
...
- [ ] **Pipeline LLM call multiplier — timeout budget** — when a PR replaces a single-call pipeline stage with a multi-stage sequence (e.g. skeleton → expand-per-chapter × N → strip → persist), verify that the integration test `TIMEOUT_SECONDS` constant is still adequate. A 4-stage outline pipeline makes at least 4× more LLM calls than a single-call baseline; a budget sized for 1 call per stage may be consumed faster than expected when combined with subsequent per-chapter write and edit stages. Check that the constant is annotated with the call-count assumption it was sized for, so future multi-stage refactors have a baseline to compare against. (Source: issue #296, PR #308.)
- [ ] **Integration test checkpoint manifest drift** — when a PR adds or removes a pipeline phase (adding or removing a savepoint key), verify that any hardcoded checkpoint name lists in integration tests (e.g. `_savepoint_names()` in `tests/integration/test_resume_granularity.py`) are updated to match. These lists are not validated against the orchestrator and silently become stale; interrupt+resume tests built on them will construct `PipelineState` objects with the wrong `savepoints` list, potentially masking phase-skip bugs. Grep `tests/integration/` for hardcoded phase or savepoint name lists and cross-reference against the checkpoint keys written by `run_pipeline`. (Source: issue #323, PR #335.)
...
- [ ] **Full-file verification for docs** — when reviewing documentation or prose files, verify content against the full file (not just diff context). Diff hunks can misrepresent line wrapping, list indentation, and code block boundaries, leading to false positives.
