...
orchestrator code to infer the pattern — but may miss the savepoint-save side-effect of
`_mark_work_item_done`, choose a `phase` key that collides with an existing phase, or add a
redundant `_write_savepoint` call immediately after `_mark_work_item_done`. All three failure
modes are silent.
...
Applied: added gotcha #048 — "Per-chapter ledger guards: use `_is_work_item_done` /
`_mark_work_item_done` for idempotent pipeline steps" — to `.github/notes/gotchas.md`
under a new "Checkpointing" section.
