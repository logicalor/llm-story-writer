<!-- STALE — archived to archive/issue-318-duplicate-ledger-helpers-2026-05-03.md — delete this file -->
---
date: "2026-05-03"
issue: 318
pr: 330
...
> extract the shared helper to a new `_<name>.py` private module in the same package. Both
> the original module and the sub-module import from the new shared module. This is the
> `src/tools/_io.py` / `src/tools/_llm.py` extraction pattern applied to `src/presentation/`.
> Duplication creates implementation divergence within a single PR and accumulates further
> divergence as future phases add their own copies.
>
...
Proposed for approval — structural change (new shared module, imports across two files, mypy
verification required). A follow-up issue should be opened; this is out of scope for the
Reflection agent's direct edit scope.
